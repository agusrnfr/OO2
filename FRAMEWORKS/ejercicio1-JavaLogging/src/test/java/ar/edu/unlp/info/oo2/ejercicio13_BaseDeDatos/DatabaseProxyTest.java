	...
	private static Logger logger = Logger.getLogger("bd");
	   ...
	   @BeforeAll
	    public static void configLogger() {
			...
			wordsToHide.add("search");
			FilterHandler filterHandler = new FilterHandler(handlerConsola, wordsToHide);
			logger.addHandler(filterHandler);
			
			MailHandler mailHandler = new MailHandler();
			mailHandler.setFormatter(new ShoutingSimpleFormatter());
			FilterHandler filterHandlerMail = new FilterHandler(mailHandler, wordsToHide);
			logger.addHandler(filterHandlerMail);
		}
